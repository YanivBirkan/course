...
function renderBoard(board: string[][]) {
  ...
  for (let i = 0; i < gBoard.length; i++) {
    ...
    for (let j = 0; j < gBoard[0].length; j++) {
      ...
      let className: string = cell ? "occupied" : "";
  ...
  strHTML += "</tr>\n";
  let elBoard = document.querySelector<HTMLElement>(".board")!;
...
function runGeneration(board: string[][]): string[][] {
  let newBoard: string[][] = copyMat(board);
  ...
  for (var i = cellI - 1; i <= cellI + 1; i++) {
    ...
    for (var j = cellJ - 1; j <= cellJ + 1; j++) {
      ...
      if (cell === LIFE) {
        // console.log("i,j:", i, j);
        gBoard[i][j] = "";
        let elCell: HTMLElement = document.querySelector(
          `[data-i="${i}"][data-j="${j}"]`,
        )!;
        elCell.innerText = "";
